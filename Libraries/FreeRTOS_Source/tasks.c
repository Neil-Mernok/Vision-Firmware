				...
				for( x = 0; x < uxArraySize; x++ )
				{
					...
					if( ulStatsAsPercentage > 0UL )
					{
						...
						#else
						{
							...
							sprintf( ( char * ) pcWriteBuffer, ( char * ) "%s\t\t%u\t\t%u%%\thigh water: %u\r\n", pxTaskStatusArray[ x ].pcTaskName, ( unsigned int ) pxTaskStatusArray[ x ].ulRunTimeCounter, ( unsigned int ) ulStatsAsPercentage, uxTaskGetStackHighWaterMark(pxTaskStatusArray[ x ].xHandle) );
						}
						#endif
					}
					else
					{
						...
						#else
						{
							...
							sprintf( ( char * ) pcWriteBuffer, ( char * ) "%s\t\t%u\t\t<1%%\thigh water: %u\r\n", pxTaskStatusArray[ x ].pcTaskName, ( unsigned int ) pxTaskStatusArray[ x ].ulRunTimeCounter, uxTaskGetStackHighWaterMark(pxTaskStatusArray[ x ].xHandle) );
						}
						#endif
					}
