...
usbd_cdc_EP0_RxReady, usbd_cdc_DataIn, usbd_cdc_DataOut, usbd_cdc_SOF, NULL, NULL, USBD_cdc_GetCfgDesc, 
#ifdef USE_USB_OTG_HS   
...
HIBYTE(CDC_CMD_PACKET_SZE),
		...
		HIBYTE(CDC_DATA_MAX_PACKET_SIZE), 0x00, /* bInterval: ignore for Bulk transfer */
...
static uint8_t usbd_cdc_DeInit(void *pdev, uint8_t cfgidx)
{
	...
	/* Restore default state of the Interface physical components */APP_FOPS.pIf_DeInit();
...
static uint8_t usbd_cdc_Setup(void *pdev, USB_SETUP_REQ *req)
{
	...
	case USB_REQ_TYPE_CLASS:
		...
		else /* No Data request */
		{
			...
			APP_FOPS.pIf_Ctrl(req->bRequest, (uint8_t*)req->wValue, 2);		// pass setup packet
		}
...
static uint8_t usbd_cdc_DataIn(void *pdev, uint8_t epnum)
{
	...
	if (USB_Tx_State == 1)
	{
		...
		else
		{
			...
			DCD_EP_Tx(pdev, CDC_IN_EP, (uint8_t*) &APP_Rx_Buffer[USB_Tx_ptr], USB_Tx_length);
		}
	}
...
static uint8_t usbd_cdc_DataOut(void *pdev, uint8_t epnum)
{
	...
	/* USB data will be immediately processed, this allow next USB traffic being 
	 NAKed till the end of the application Xfer */APP_FOPS.pIf_DataRx(USB_Rx_Buffer, USB_Rx_Cnt);
