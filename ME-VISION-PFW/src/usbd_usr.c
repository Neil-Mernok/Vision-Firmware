...
//	DCD_DevDisconnect(&USB_OTG_dev);
	...
	gccfg.d32 = USB_OTG_dev.regs.GREGS->GCCFG;   
	
	gccfg.b.vbussensingA = 0 ;
	gccfg.b.vbussensingB = 0 ;     
	gccfg.b.disablevbussensing = 0; 
	
	USB_OTG_dev.regs.GREGS->GCCFG = gccfg.d32;
	
	USB_plugged(false);
}

/**
* @brief  USB_OTG_Disable_VBUS_sense
*         Kobus: atempt to set the device's Bus sensing off after USB is enumerated. 
*/
void USB_OTG_Disable_sensing(USB_OTG_CORE_HANDLE *pdev)
{	
	USB_OTG_GCCFG_TypeDef    gccfg;
...
//	GPIO_ResetBits(USB_Pull_PORT, USB_Pull_PIN);		// enable pull-up
	// seems to only be called when plugged in. 
	USB_plugged(true);
}
