...
/**
 * Create a pipe to holde fifo data. 
 * @param p pipe
 * @param length number of elements
 * @param obj_size size of element
 * @return
 */
int pipe_init(kpipe* p, int length, int obj_size)
{
	p->buf = malloc(obj_size*(length+1));
	
	if(p->buf == NULL)		// error in Malloc creation process. 
		return -1;
	
	p->end = p->buf + obj_size*(length-1);	
	p->get = p->put = p->buf;
	p->item_size = obj_size;
	p->count = 0;
	p->max = length;
	return 0;
}
...
/**
 * Add an item to the pipe. FIFO.
 * @param p pipe
 * @param data item
 * @return
 */
int pipe_put(kpipe* p, void* data)
{
	if(p->buf == NULL)		// error in Malloc creation process. 
		return -1;
...
/**
 * Push a priority item to the start of the pipe. (skip the queue)
 * @param p pipe
 * @param data item
 * @return
 */
int pipe_push(kpipe* p, void* data)
{
	if(p->buf == NULL)		// error in Malloc creation process. 
		return -1;
		
	p->count++;
	p->get -= p->item_size;	// change the pointer to the start of the fifo
	if(p->get < p->buf)
		p->get = p->end;
	memcpy(p->get, data, p->item_size);
	return 0;
}

int pipe_get(kpipe* p, void* data)
{
	if(p->put == p->get)
		return 0;
	else
...
/**
 * @brief: gets the first item on the list, but wont increment the pointers. 
 * @param p
 * @param data
 * @return
 */
int pipe_peek(kpipe* p, void* data)
{
	if(p->put == p->get)
		return 0;
	else
