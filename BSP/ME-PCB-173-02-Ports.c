...
{
	GPIO_PinState R = LED_OFBIT, G = LED_OFBIT, B = LED_OFBIT;
...
//		MH = GPIOH->MODER;
	...
	if (sleep)
	{
		...
		GPIOA->MODER = 0b11101001111111111111110111111111;
		// LC_CLK (B3), GPIOs(B8-11)
		GPIOB->MODER = 0b11111111010101011111111100111111;
		// set GPO's low
		GPIOB->ODR &= 0xF0FF;
...
void Start_EXTI_LFDAT(pFunction callback)
{
	...
	GPIO_InitStruct.Mode = GPIO_MODE_IT_RISING;
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(LF_DAT_IRQ_PORT, &GPIO_InitStruct);
...
void Stop_EXTI_LFDAT(void)
{
	...
	HAL_GPIO_DeInit(LF_DAT_PORT, LF_DAT_PIN);
